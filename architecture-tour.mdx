...
`auth/context.py` is where `@require_auth` lives: it wraps a remote function and raises `AuthRequired`, a `RemoteError` the router serializes to a 401. `auth/public.py` holds `@public_identity`, the one projection an app registers to decide what crosses to the client as the `$fymo/auth` store. Route-level enforcement (`require_auth:` in `fymo.yml`) is handled separately, in `core/page_auth.py`, since it runs before SSR starts rather than inside a remote-function call.
