...
`auth/context.py` is where request-scoped identity lives. `current_user()` reads and verifies the `fymo_session` cookie. Then it looks the user up through whichever `UserStore` is configured. `@require_auth` wraps a remote function and raises a `RemoteError`, serialized to a 401, when there's no user.

Login and signup don't return a `Set-Cookie` header directly. Instead they queue one through `queue_session_cookie()`, and `remote/router.py` drains that queue after the call returns. The session-resolver chain, `register_session_resolver`, is the seam other providers, like an OAuth or token-based one, plug into without needing a `fymo_session` cookie at all.
...
`storage/registry.py` builds a single `StorageProvider`. It reads that provider's config from the `storage:` section of `fymo.yml`. Unlike auth, jobs, or broadcasts, storage has no default.

An app with `media:` configured but no `storage:` section fails at build time, and again at startup. That's safer than quietly writing files to local disk in dev, only to lose them once the app runs behind more than one instance in production.
